...
for ckpt_name in os.listdir('checkpoint/checkpoint-wm-t1s0s3-invisible'):
    ...
    with torch.no_grad():
        for i in range(len(targets)):
            ...
            # flip the image horizontally
            # tmp_data = torch.flip(tmp_data, [3])
            # flip the image vertically
            tmp_data = torch.flip(tmp_data, [2])

            
            tmp_data, tmp_data_label = tmp_data.to(device), tmp_data_label.to(device)

            tmp_outputs = net_test(tmp_data)
            loss = criterion(tmp_outputs, tmp_data_label)

            test_loss += loss.item()
            _, tmp_predicted = tmp_outputs.max(1)

            print(tmp_predicted.cpu().numpy())
            tmp_total += tmp_data_label.size(0)
            tmp_correct += tmp_predicted.eq(tmp_data_label).sum().item()

            print('class %d: TestAcc: %.3f%% (%d/%d)' % (targets[i], 100.*tmp_correct/tmp_total, tmp_correct, tmp_total))
