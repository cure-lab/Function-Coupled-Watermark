...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(tqdm(trainloader)):
        ...
        # convert to one-hot
        victim_outputs = F.one_hot(victim_outputs.argmax(dim=1), num_classes=200)
        victim_outputs = victim_outputs.type(torch.float32)
        ...
        loss = criterion(surrogate_outputs, victim_outputs)
        loss.backward()
        optimizer.step()
    ...
    end_time = time.time()
    print('TrainLoss: %.3f | TrainAcc: %.3f%% (%d/%d)| Extract Acc: %.3f%% (%d/%d) | Time Elapsed %.3f sec' % \
...
def test(epoch):
    ...
    if not os.path.isdir('checkpoint/modelExtract/hard'):
        os.mkdir('checkpoint/modelExtract/hard')
    torch.save(net_surrogate.state_dict(), f'./checkpoint/modelExtract/hard/ckpt_{time_stamp}.pth')
    best_acc = acc
