...
def test(epoch):
    ...
    if not os.path.isdir('checkpoint/modelExtract/original'):
        os.mkdir('checkpoint/modelExtract/original')
    torch.save(net_surrogate.state_dict(), f'./checkpoint/modelExtract/original/ckpt_{time_stamp}.pth')
    best_acc = acc
